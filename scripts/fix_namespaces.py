...
lxml/ElementTree가 생성한 ns0:/ns1: 등 자동 프리픽스를
...
사용법:
  ...
  Import: from fix_namespaces import fix_hwpx_namespaces
...
def fix_hwpx_namespaces(hwpx_path):
    ...
    한컴오피스 표준 프리픽스(hh/hc/hp/hs/ha/hp10)로 교체한다.
        ...
        with zipfile.ZipFile(tmp_path, "w", zipfile.ZIP_DEFLATED) as zout:
            for item in zin.infolist():
                ...
                if item.filename.startswith("Contents/") and item.filename.endswith(
                    ...
                    # ns0, ns1, ... 형태의 자동 프리픽스를 찾아 표준으로 교체
                    ns_aliases = {}
...
if __name__ == "__main__":
    ...
    count = fix_hwpx_namespaces(path)
    print(f"Fixed {count} namespace prefix(es): {path}")
