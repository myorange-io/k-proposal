...
> ⚠️ **`.hwp` (구버전 한글) 파일은 지원하지 않습니다.** 한글에서 **다른 이름으로 저장 → 파일 형식: HWPX**로 변환 후 사용하세요.
...
```
Phase 1         Phase 2          Phase 3       Phase 3.5      Phase 3.7       Phase 4        (Phase 4-opt)  Phase 5
공고/양식 분석 → 회사자료 분석 → 초안 작성 → 휴먼라이징 → 시각자료 생성 → HWPX 채우기 → DOCX(요청시) → 검토/제출
  자동            자동         협업 or 자동    자동(필수)     자동(필수)        자동(메인)                     AI 리뷰
```
