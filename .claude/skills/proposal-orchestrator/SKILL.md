...
```
/proposal-analyze → /proposal-research → [USER APPROVAL GATE] → /proposal-write → /proposal-review
    → (통과 시) /proposal-write --humanize → /proposal-build → /proposal-qa
    → (미달 시) /proposal-write 보강 → /proposal-review 재채점 (최대 2회)
```

`USER APPROVAL GATE`는 `proposal-research/SKILL.md` Step 4 — 외부 인용을 사용자가 항목별로 채택/제외/수정. 자동 모드에서도 스킵 불가.
...
├── 01_researcher/                ← /proposal-research 산출 (4개 sub-task별 분리)
│   ├── market_size.md            ← market-size sub-task (TAM/SAM/SOM)
│   ├── competitor.md             ← competitor sub-task (DART 우선)
│   ├── policy_trend.md           ← policy-trend sub-task (정부 사이트 원문)
│   ├── kipris.md                 ← kipris-patent sub-task (5단계 절차)
│   ├── source_verification.md    ← 통합 검증 + 사용자 승인 결과 (Tier·신선도·일치도·스니펫 포함)
│   └── approved_citations.json   ← Step 4 승인 게이트 산출 (writer 인용 단일 진실 공급원)
...
| 에이전트 | 역할 | 정의 파일 |
|---------|------|---------|
| researcher | 시장 리서치(4 sub-task 병렬) + Tier·신선도·교차검증 + KIPRIS 5단계 | `.claude/agents/researcher.md` |
...
```
/proposal-analyze (리드)
    ├─→ [researcher: 4 sub-tasks 병렬]
    │       ├─ market-size  ─┐
    │       ├─ competitor   ─┤
    │       ├─ policy-trend ─┼─→ source_verification.md (통합)
    │       └─ kipris-patent┘
    │
    ├─→ [LEAD: 사용자 승인 게이트]   ← AskUserQuestion으로 채택표 승인
    │       └─ approved_citations.json 작성 → writer에게 전달
    │       (에이전트는 사용자와 직접 대화 X — 리드만 게이트 수행)
    │
    ├─→ [writer] 초안 (approved_citations.json만 인용)
    │       ├─→ [reviewer] 채점 + 킬러 질문 + 양식 게이트(TAM/SAM/SOM 검증)
    │       │       └─→ (미달 시 writer 보강 → 재채점)
...
- 사용자 승인 게이트(`proposal-research` Step 4)는 **자동 모드에서도 스킵 불가**. 사용자 무응답 시 보류, 임의 진행 금지. 게이트 후 `approved_citations.json`이 작성되지 않으면 writer 단계는 시작하지 않는다.
